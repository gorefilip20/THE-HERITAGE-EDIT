...
const SUPPORT_NAV = [
  ...
  { label: "Issues", href: "/support/issues", icon: AlertCircle },
...
export default function SupportLayout({ children }: SupportLayoutProps) {
  ...
  return (
    <div className="flex h-screen bg-neutral-50">
      {/* Sidebar */}
      <aside
        className={`fixed md:relative z-40 h-screen w-64 bg-purple-900 text-white transition-transform duration-300 ${
          sidebarOpen ? "translate-x-0" : "-translate-x-full md:translate-x-0"
        }`}
      >
        <div className="p-6 border-b border-white/10">
          <h1 className="text-xl font-serif tracking-wide">Support Hub</h1>
          <p className="text-xs font-sans text-white/50 mt-1">Customer Service</p>
        </div>
      ...
      <button
        ...
        className="md:hidden fixed bottom-6 right-6 z-50 w-12 h-12 bg-purple-900 text-white flex items-center justify-center rounded-full shadow-lg"
      >
